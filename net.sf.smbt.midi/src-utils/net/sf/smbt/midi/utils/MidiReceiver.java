...
public class MidiReceiver implements	Receiver
{
	...
	MidiInputJob inputJob;


	public MidiReceiver(MidiInputJob inputJob, PrintStream printStream) {
		this(inputJob, printStream, false);
	}


	public MidiReceiver(MidiInputJob inputJob, PrintStream printStream, boolean bPrintTimeStampAsTicks) {
	...
	public void send(MidiMessage message, long lTimeStamp) {
		for (CmdPipe rx : inputJob.rxList) {
			synchronized (rx) {
				if (rx.isActivated()) {
					rx.receive(message.getMessage());
				}
			}
		}
