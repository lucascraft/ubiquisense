...
public class RXTXSerialUtil implements IRXTXSerialUtils {
	 ...
	 */
	public ISerialCommunicator openPort(final Port port, final String portID, int speed) {
	...
	final class SerialInputJob extends Job {
						...
						if (engineMap.get(portID) != null) {
							...
							if (p.getChannel() != null) {
								...
								for (IQxEventHandler h : p.getChannel().getEventListeners()) {
									try {
										Event e = EngineFactory.eINSTANCE.createEvent();
