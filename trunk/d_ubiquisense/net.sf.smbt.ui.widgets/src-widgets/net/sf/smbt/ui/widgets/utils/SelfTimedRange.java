...
package net.sf.smbt.ui.widgets.utils;
...
public class SelfTimedRange <T extends TimedValue> {
	...
	public SelfTimedRange(final long scope) {
	...
	public void setTimedValue(T value) {
		if (value != null && value.getD() != null) {
			...
			if (value.getD().before(oldestSamplingDate)) 		{ oldestSamplingDate 	= value.getD(); 		}	
			if (value.getD().after(latestSamplingDate)) 		{ latestSamplingDate 	= value.getD(); 		}
			if (value.getVolume() < minVolumeSampling || minVolumeSampling == 0) 	{ minVolumeSampling		= value.getVolume(); 	}
			if (value.getVolume() > maxVolumeSampling) 								{ maxVolumeSampling 	= value.getVolume(); 	}
			if (value.getV() < minValueSampling || minValueSampling == 0) 			{ minValueSampling 		= value.getV(); 		}
			if (value.getV() > maxValueSampling) 									{ maxValueSampling 		= value.getV(); 		}
	...
	private synchronized Map<Date, PositionningValue> _getPositions(double w, double h, Collection<T> timedValues){
		...
		if (!timedValues.isEmpty()) {
			...
			for (TimedValue v : timedValues) {
				...
				v.setPosition(new Point(w * hRatio, h * vRatio));
				points.put(v.getD(), v);
			}
		}
		return points;
