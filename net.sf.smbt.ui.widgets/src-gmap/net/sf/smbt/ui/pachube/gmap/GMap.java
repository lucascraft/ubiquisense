 ...
 *     Lucas Bigeardel - additional features
 ******************************************************************************/
...
public class GMap extends Composite {
  ...
  public void addGpxFromString( final String gpxString ) {
	checkWidget();
	if ( loaded ) {
		
		String escaped = StringEscapeUtils.escapeXml(gpxString);
		browser.evaluate( "addGpxFromString ( \"" + escaped + "\" )" );
	}
  }
  
  public void addKmlFromString( final String kmlString ) {
	checkWidget();
	if ( loaded ) {
		String escaped = StringEscapeUtils.escapeXml(kmlString);
		browser.evaluate( "addKmlString ( \"" + escaped + "\" )" );
	}
  }
  
  public void addTrace( final String tracePtArray ) {
	  checkWidget();
	  if( loaded ) {
		 browser.evaluate( "addTrace( " + tracePtArray + " );" );
	  }
  }
   ...
   */
  public void selectMarker( final LatLng position, final String title ) {
	  if (position != null) {
		  checkWidget();
		  browser.evaluate( "selectMarker( [" + position.toString() + "],\"" + title + "\" )" );
	  }
  }
  ...
  public void addMarkerDetail( final String compactString ) {
	try {
		checkWidget();
		browser.evaluate( "addMarkerDetail( \"" + compactString + "\")" );
	} catch (Exception e) {
		e.printStackTrace();
	}
  }
  ...
  public void addMarkers( final String compactString ) {
	try {
		checkWidget();
		browser.evaluate( "addMarkerList( \"" + compactString + "\")" );
	} catch (Exception e) {
		e.printStackTrace();
	}
  }

  public void importKML( final String kml ) {
	try {
		checkWidget();
		browser.evaluate( "addKML( \"" + kml + "\")" );
	} catch (Exception e) {
		e.printStackTrace();
	}
  }

  public void addCircle( final String lat, final String lon, final String dist ) {
	try {
		checkWidget();
		browser.evaluate( "addCircle( " + dist + ")" );
	} catch (Exception e) {
		e.printStackTrace();
	}
  }
  ...
  private void createBrowserFunctions() {
    ...
    new BrowserFunction( browser, "onMarkerSelected" ) {
      public Object function( Object[] arguments ) {
        onMarkerSelected( ( String ) arguments[ 0 ] );
        return null;
      }
    };
  }
  ...
  private void fireMarkerSelected(String id) {
    Object[] allListeners = listeners.getListeners();
    for( int i = 0; i < allListeners.length; i++ ) {
      MapListener listener = ( MapListener ) allListeners[ i ];
      listener.markerSelected(id);
    }
  }
